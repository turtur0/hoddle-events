...
export function calculateRawPopularityScore(event: IEvent): number {
  ...
  if (event.externalPopularity?.spotify) {
    const spotifyScore = calculateSpotifyScore(
      event.externalPopularity.spotify.popularity
    );
  ...
  const venueCapacity =
    VENUE_CAPACITIES[event.venue.name] ||
    estimateVenueCapacity(event.venue.name);
  ...
  const daysSinceListed =
    (Date.now() - event.scrapedAt.getTime()) / (1000 * 60 * 60 * 24);
 ...
 */
export async function enrichWithSpotifyData(
  batchSize: number = 50
): Promise<number> {
  ...
  for (const event of events) {
    try {
      ...
      if (spotifyArtist) {
        ...
        enriched++;
        console.log(
          `[Spotify] OK ${event.title} -> ${spotifyArtist.name} (${spotifyArtist.popularity}/100)`
        );
      } else {
        failed++;
      }
...
export async function updateCategoryPopularityPercentiles(): Promise<void> {
  ...
  for (const category of CATEGORIES.map((cat) => cat.value)) {
    ...
    console.log(
      `[PopularityService] Processing ${events.length} events in ${category}`
    );
    ...
    console.log(
      `[PopularityService] Updated ${totalEvents} events in ${category}`
    );
  }
